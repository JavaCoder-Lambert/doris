...
后续 4.1.4 验证补充：相同 ASAN ELF 在真正 x86_64 Linux 内核上已完整执行定向测试，四个新增 RPC 用例分别复现 response 污染、Controller 污染、回调前 attachment 未清理及 callback 销毁后 attachment 未清理。它们是确定性的回调重入/资源契约测试，不是网络并发或用户事故复现。最小适配已提交并推送为 `8048d149`，修复后的 11 项 focused ASAN 单测全部通过、无跳过；精确 ELF/XML 摘要及完整范围见 [4.1.4 交付记录](4.1.4-null-safe-fix.md)。
